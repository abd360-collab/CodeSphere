...
const jwt = require('jsonwebtoken'); // library to create and verify JWT tokens.
const { validationResult } = require('express-validator'); // validates request body
...
const register = async (req, res) => {
  try {
    ...
    // No Mongoose hook → manual hashing
...
const getMe = async (req, res) => {
  try {
    // req.user.id comes from your fixed auth middleware
    const user = await prisma.user.findUnique({
